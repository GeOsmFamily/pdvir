<template>
    ...
    <v-menu v-else class="cursor-pointer">
      ...
      <v-list>
        <v-list-item v-if="userStore.currentUser?.roles.includes(UserRoles.EDITOR_PROJECTS) || userStore.currentUser?.roles.includes(UserRoles.ADMIN)">
        ...
        <v-list-item v-if="userStore.currentUser?.roles.includes(UserRoles.EDITOR_RESOURCES) || userStore.currentUser?.roles.includes(UserRoles.ADMIN)">
            <template v-slot:prepend>
                <v-icon color="main-blue" icon="mdi-pencil-outline"></v-icon>
            </template>
          <v-list-item-title>{{ $t('header.addResource') }}</v-list-item-title>
        </v-list-item>
        <v-list-item v-if="userStore.currentUser?.roles.includes(UserRoles.EDITOR_DATA) || userStore.currentUser?.roles.includes(UserRoles.ADMIN)">
            <template v-slot:prepend>
                <v-icon color="main-blue" icon="mdi-pencil-outline"></v-icon>
            </template>
        ...
        <v-list-item v-if="isEditor">
            <template v-slot:prepend>
                <v-icon color="main-blue" icon="mdi-tune"></v-icon>
            </template>
          <v-list-item-title>{{ $t('header.content') }}</v-list-item-title>
        </v-list-item>
        <v-list-item v-if="userStore.currentUser?.roles.includes(UserRoles.ADMIN)" to="/administration">
            <template v-slot:prepend>
                <v-icon color="main-blue" icon="mdi-tune"></v-icon>
            </template>
...
const isEditor = computed(() => {
    return userStore.currentUser 
        && (userStore.currentUser.roles.includes(UserRoles.EDITOR_PROJECTS) 
            || userStore.currentUser.roles.includes(UserRoles.EDITOR_ACTORS)
            || userStore.currentUser.roles.includes(UserRoles.EDITOR_RESOURCES)
            || userStore.currentUser.roles.includes(UserRoles.EDITOR_DATA))
})
</script>