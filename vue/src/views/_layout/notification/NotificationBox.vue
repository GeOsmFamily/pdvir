<template>
  <v-snackbar
    ...
    <p class="NotificationBox__desc" v-if="activeNotification?.description">{{ activeNotification?.description }}</p>
  </v-snackbar>
</template>
...
const notificationTime = computed(() => activeNotification.value?.type === NotificationType.ERROR ? 7000 : 2500)
...
watch(() => appStore.notificationPile, async () => {
