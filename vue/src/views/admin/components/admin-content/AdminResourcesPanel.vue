        ...
        >
            <template #editContentCell="{ item } " >
                <template v-if="!item.isValidated">
                    <v-btn size="small" icon="mdi-arrow-right" class="text-main-blue" @click="editResource(item as Resource, FormType.VALIDATE)"></v-btn>
                </template> 
                <template v-else>
                    <v-btn density="comfortable" icon="mdi-pencil-outline" @click="editResource(item as Resource)" class="mr-2"></v-btn>
                    <v-btn density="comfortable" icon="mdi-dots-vertical">
                        ...
                        <v-menu activator="parent" location="left">
                            <v-list class="AdminPanel__additionnalMenu">
                                <v-list-item @click="resourceStore.deleteResource(item as Resource)">{{ $t('resources.admin.delete')}}</v-list-item>
                            </v-list>
                        </v-menu>
                    </v-btn>
                </template>
