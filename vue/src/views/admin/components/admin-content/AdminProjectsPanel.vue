        ...
        >
            <template #editContentCell="{ item } " >
                <template v-if="!item.isValidated">
                    <v-btn size="small" icon="mdi-arrow-right" class="text-main-blue" @click="editProject(item as Project, FormType.VALIDATE)"></v-btn>
                </template> 
                <template v-else>
                    <v-btn density="comfortable" icon="mdi-pencil-outline" @click="editProject(item as Project)" class="mr-2"></v-btn>
                    <v-btn density="comfortable" icon="mdi-dots-vertical">
                        ...
                        <v-menu activator="parent" location="left">
                            <v-list class="AdminPanel__additionnalMenu">
                                <v-list-item :to="`/projects/${(item as Project).slug}`">{{ $t('projects.admin.goToPage')}}</v-list-item>
                                <v-list-item @click="projectStore.deleteProject(item as Project)">{{ $t('projects.admin.delete')}}</v-list-item>
                            </v-list>
                        </v-menu>
                    </v-btn>
                </template>
...
const filteredProjects = computed(() => {
    ...
    return projects.value.filter((item: Project) => {
        return (item.actor.acronym ? item.actor.acronym.toLowerCase().includes(searchQuery.value.toLowerCase()): false) ||
