...
axiosInstance.interceptors.response.use(undefined, (error: AxiosError) => {
  switch (error.response?.status) {
    case 401:
      ...
      break;
    case 422:
      addNotification(i18n.t("notifications.common.error.411"), NotificationType.ERROR, `${i18n.t("notifications.common.error.moreDetails")} ${((error.response.data as any)?.description ?? '')}`)
      break;
    case 500:
