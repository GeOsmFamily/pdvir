...
export function localizeDate(date: string | Date, format: Intl.DateTimeFormatOptions = { year: 'numeric', month: 'long', day: 'numeric' }) {
  date = new Date(date)
  return date.toLocaleDateString('fr-FR', format)
}

export function localizeTime(date: string | Date, format: Intl.DateTimeFormatOptions = { hour: 'numeric', minute: 'numeric' }) {
  date = new Date(date)
